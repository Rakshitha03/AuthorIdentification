# This script is used to generate the .arff file to be used for
...
main_path = '/home/rakshitha03/UMass/Sem1/NLP/Project/maildir'
author_names = ['mann-k', 'kaminski-v']
for feature in lexical_features:
    print '@attribute\t\'' + feature + '\'\treal'
for feature in syntactic_features:
    print '@attribute\t\'' + feature + '\'\treal'
for feature in structural_features:
    if feature == 'struct_has_greeting':
        print '@attribute\t\'' + feature + '\'\tstring'
    else:
        print '@attribute\t\'' + feature + '\'\treal'
for name in author_names:
    count = 0
    mail_path = main_path + '/' + name + '/_sent_mail'
    for email in os.listdir(mail_path):
        ...
        struc_features = feature_extractor.extract_structural_features(
            content)
        for feature in lexical_features:
            combined_features.append(str(l_features[feature]))
        for syn_feature in syntactic_features:
            combined_features.append(str(syn_features[syn_feature]))
        for struct_feature in structural_features:
            combined_features.append(str(struc_features[struct_feature]))
        features = ','.join(combined_features)
        features += ',' + name
        print features
        count += 1
